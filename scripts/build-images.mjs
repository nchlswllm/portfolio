...
function main() {
    ...
    for (const [image, displayWidth] of byImage) {
        ...
        const outputsComplete = tiers.every((w) =>
            ["avif", "webp"].every((ext) => {
                const output = join(OUT_DIR, `${base}-${w}.${ext}`);
                return existsSync(output) && statSync(output).mtimeMs >= stat.mtimeMs;
            })
        );
        const upToDate =
            !FORCE &&
            ((prevEntry &&
                ...
                outputsComplete) ||
                (!prevEntry && outputsComplete));

        if (upToDate) {
            manifest[image] = prevEntry || { base, w: nativeW, h: nativeH, tiers, srcKey };
            skipped++;
            writeManifest();
            continue;
        }
